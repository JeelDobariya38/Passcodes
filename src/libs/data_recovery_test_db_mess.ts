import { OLD_MISTAKEN_DATABASE_NAME } from "@/config";
...
export default async function dataRecoveryFromTestDB(
  ...
  try {
    oldDb = await SQLite.openDatabaseAsync(OLD_MISTAKEN_DATABASE_NAME);
    ...
    if (!tableExists) {
      ...
      await SQLite.deleteDatabaseAsync(OLD_MISTAKEN_DATABASE_NAME);
      return;
    }
